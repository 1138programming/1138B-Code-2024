...
void default_constants() {
  // P, I, D, and Start I
  chassis.pid_drive_constants_set(12, 0.0, 5.0);         // Fwd/rev constants, used for odom and non odom motions
  chassis.pid_heading_constants_set(5.0, 0.0, 10.0);        // Holds the robot straight while going forward without odom
  chassis.pid_turn_constants_set(1.0, 0.05, 5.0, 15.0);     // Turn in place constants
...
void localAWP() {
    ...
    chassis.odom_pose_set({-60_in,-24_in,270_deg});
    chassis.pid_odom_set({{-30_in, -24_in}, rev, DRIVE_SPEED});
    chassis.pid_wait();
    MogoMech.clamp();
    pros::delay(500);
    Intake.In();
    chassis.pid_odom_set({{-24_in, -48_in}, fwd, DRIVE_SPEED});
    chassis.pid_wait();
}